...
class MetadataParser(object):
    ...
    def validate_metadata_columns(cls, meta_data):
        ...
        if len(invalid_cols) > 0:
            utils.Debug.vprint(
                "Skipping metadata because parsing requires missing columns: "
                f"{invalid_cols}",
                level=0
            )
            return False
    ...
    def create_sample_name_column(cls, meta_data):
        ...
        if cls.cond_col not in meta_data:
            ...
            utils.Debug.vprint(
                "Meta data sample name column and meta_data index are not "
                "equal",
                level=2
            )
    ...
    def fix_NAs(data_frame):
        """
        Replace the string NA with a pd.NA
...
class MetadataParserBranching(MetadataParser):
    """
    Metadata parser that handles prev_col & del_t metadata
    (which allows for Branching Time Courses)
    """

    name = "Branching"

    # Column of booleans for "Is a time-series experiment"
    ists_col = ISTS_COLUMN_NAME
    # Column of sample names (matching expression data column names)
    cond_col = COND_COLUMN_NAME
    # Column that identifies the previous timepoint sample name
    prev_col = PREV_COLUMN_NAME
    # Column that identifies the delta time between this sample and the
    # previous sample
    ...
    def process_groups(cls, meta_data):
        """
        Parse the metadata to identify steady-state experiments
        and link timecourse experiments

        :param meta_data: pd.DataFrame
        :return steady_idx, ts_group: dict, dict
            steady_idx:  Dict keyed by condition, value is boolean
                (True if the condition is a steady-state experiment)
            ts_group: Dict keyed by condition.
                [(Previous_condition_name, Previous_delt),
                 (Following_condition_name, Following_delt)]
        """
        time_series = meta_data[cls.ists_col].values.astype(bool)
        steadies = dict(
            zip(
                meta_data[cls.cond_col].astype(str),
                np.logical_not(time_series)
            )
        )
    ...
    def check_for_dupes(
        ...
        """
        Sanity check the metadata and experimental data to ensure that they
        can be linked. Try to guess as much as possible unless strict checking
        is on
        """
        # Check to make sure that the conditions in the expression data are
        # matched with conditions in the metadata
        exp_conds = exp_data.columns.astype(str)
        meta_conds = meta_data[cls.cond_col].astype(str)

        # Check to find out if there are any conditions which don't have
        # associated metadata
        # If there are, raise an exception if strict_checking_for_metadata
        # is True. Otherwise just assume they're steady-state data and move on
        in_exp_not_meta = exp_conds.difference(meta_conds).tolist()
        if len(in_exp_not_meta) != 0:
            utils.Debug.vprint(
                f"{len(in_exp_not_meta)} conditions cannot be properly "
                "matched to metadata",
                level=1
            )
            utils.Debug.vprint(" ".join(in_exp_not_meta), level=2)
            if strict_checking_for_metadata:
                raise ConditionDoesNotExistError(
                    "Conditions exist without associated metadata"
                )
            else:
                for condition in in_exp_not_meta:
                    steady_idx[condition] = True

        # Check to find out if the conditions in the expression data
        # are all unique
        # It's not a problem if they're not, we just assume the metadata
        # applies to all conditions with the same name
        duplicate_in_exp = exp_conds.duplicated()
        n_dup_in_exp = np.sum(duplicate_in_exp)
        if n_dup_in_exp > 0:
            c_dup = exp_conds[duplicate_in_exp].tolist()
            utils.Debug.vprint(
                f"The expression data has {n_dup_in_exp} non-unique "
                "condition indexes",
                level=1
            )
        ...
        # If there are repeats, check and see if the associated information is
        # identical (if it is, just ignore it)
        # If there are repeated conditions with different characteristics, the
        # outcome may be unexpected
        # (The parser will just overwrite the first ones it comes to with the
        ...
        if n_dup_in_meta > 0:
            ...
            if np.sum(
                np.logical_xor(meta_data.duplicated(), duplicate_in_meta)
            ) > 0:
                utils.Debug.vprint(
                    "The metadata has non-unique conditions with different "
                    "characteristics:",
                    level=0
                )
                utils.Debug.vprint(" ".join(meta_dup), level=0)
                if strict_checking_for_duplicates:
                    raise MultipleConditionsError(
                        "Identical conditions have non-identical "
                        "characteristics"
                    )
            else:
                utils.Debug.vprint(
                    "fThe metadata contains {n_dup_in_meta} duplicate rows",
                    level=1
                )
    ...
    def validate_metadata(cls, exp_data, meta_data):
        ...
        # Check and make sure that there's a name column and create it
        # if needed
        cls.create_sample_name_column(meta_data)
        ...
        if align_count == 0:
            raise ConditionDoesNotExistError(
                "Unable to align metadata to expression data"
            )
        elif align_count < min(exp_data.shape[1], meta_data.shape[0]):
            utils.Debug.vprint(
                f"Metadata {meta_data.shape} and "
                f"expression data {exp_data.shape} alignment off",
                level=0
            )
    ...
    def create_default_meta_data(cls, metadata_index):
        ...
        # Create an empty dataframe with index equal to sample names
...
class MetadataParserNonbranching(MetadataParserBranching):
    ...
    def process_groups(cls, meta_data):
        ...
        times_per_group = {
            k: []
            for k in meta_data[cls.group_col].unique()
        }
        group_time_cond = {
            k: {}
            for k in meta_data[cls.group_col].unique()
        ...
        # Process into a dict, keyed by sample ID, of
        # [(previous sample, del.t), (next sample, del.t)]
        for cond, (group, time) in ts_dict.items():
            group_times = pd.Series(times_per_group[group])

            if (
                steady_idx[cond] or
                (min(group_times) == time and max(group_times) == time)
            ):
                steady_idx[cond] = True
                ts_group.pop(cond)
                continue
            ...
            if min(group_times) == time:
                ...
                prev_time = max(
                    [
                        ...
                        for (i, v) in zip(
                            group_times,
                            map(lambda x: x < time, group_times)
                        )
                        if v
                    ]
                )
...
class MetadataHandler(object):
    ...
    def get_handler(cls, handler_ref):
        """
        This wrappers a metadata reference so that strings can be used
        instead of python imports
        ...
        :return: MetadataParser
            The metadata parser that corresponds to the string, or the
            MetadataParser object will be passed through
        """
        if utils.is_string(handler_ref):
            ...
            raise ValueError(
                "Handler must be a string or a MetadataParser class"
            )
