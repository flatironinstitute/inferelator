...
class TestResultsProcessor(unittest.TestCase):
    ...
    def test_combining_confidences_one_beta_with_negative_values(self):
        ...
        beta = pd.DataFrame(np.array([[-0.2841755, 0, 0.2280624, -0.3852462, 0.2545609]]), ['gene1'],
                            ['tf1', 'tf2', 'tf3', 'tf4', 'tf5'])
        rescaled_beta = pd.DataFrame(np.array([[0.09488207, 0, 0.07380172, 0.15597205, 0.07595131]]), ['gene1'],
                                     ['tf1', 'tf2', 'tf3', 'tf4', 'tf5'])
        confidences = model_performance.RankSummaryPR.compute_combined_confidences([rescaled_beta])
        np.testing.assert_equal(confidences.values,
    ...
    def test_combining_confidences_two_betas_negative_values(self):
        # data was taken from a subset of row 42 of b subtilis run
        beta1 = pd.DataFrame(np.array([[-0.2841755, 0, 0.2280624, -0.3852462, 0.2545609]]), ['gene1'],
                             ['tf1', 'tf2', 'tf3', 'tf4', 'tf5'])
        rescaled_beta1 = pd.DataFrame(np.array([[0.09488207, 0, 0.07380172, 0.15597205, 0.07595131]]), ['gene1'],
                                      ['tf1', 'tf2', 'tf3', 'tf4', 'tf5'])
        beta2 = pd.DataFrame(np.array([[0, 0.2612011, 0.1922999, 0.00000000, 0.19183277]]), ['gene1'],
                             ['tf1', 'tf2', 'tf3', 'tf4', 'tf5'])
        rescaled_beta2 = pd.DataFrame(np.array([[0, 0.09109101, 0.05830292, 0.00000000, 0.3675702]]), ['gene1'],
                                      ['tf1', 'tf2', 'tf3', 'tf4', 'tf5'])
        confidences = model_performance.RankSummaryPR.compute_combined_confidences([rescaled_beta1, rescaled_beta2])
        np.testing.assert_equal(confidences.values,
